...
export class UserService {
  ...
  private products = new BehaviorSubject<Product[]>(this.loadProductsFromStorage() || [
  ...
  addUser(name: string, email: string, role: UserRole): { user: User, isRepeated: boolean } {
    ...
    // Set the newly created user as the current user so guards and other
    // parts of the app see the authenticated user immediately.
    this.currentUser.next(user);
    this.saveToStorage();
